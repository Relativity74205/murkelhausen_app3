...
def _replace_empty_str_with_none(v: str) -> str | None:
    if v == "":
        return None
    return v
...
def get_vertretungsplan(vertretungsplan_date: date) -> Vertretungsplan:
    ...
    for event in data["events"]:
        ...
        event = VertretungsplanEvent(
            ...
            previousRoom=_replace_empty_str_with_none(event["previousRoom"]),
            previousSubject=_replace_empty_str_with_none(event["previousSubject"]),
            previousTeacher=_replace_empty_str_with_none(event["previousTeacher"]),
            room=_replace_empty_str_with_none(event["room"]),
            subject=_replace_empty_str_with_none(event["subject"]),
            teacher=_replace_empty_str_with_none(event["teacher"]),
            comment=text,
...
def get_full_class_of_mattis() -> str:
    ...
    current_jahrgang = current_year - MATTIS_YEAR_STARTED + 5
    return f"{current_jahrgang}{MATTIS_CLASS_SUFFIX}"
